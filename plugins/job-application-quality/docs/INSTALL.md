...
"./plugins/job-application-quality" }`. That path is a materialized repo-local plugin package path,
so a sparse checkout containing only `.agents/plugins` is not enough for installation. It must not
be a symlink to the repo root because local private data directories are intentionally gitignored at
the root.

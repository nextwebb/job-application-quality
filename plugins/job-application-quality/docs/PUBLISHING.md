...
- `plugins/job-application-quality/` is a real packaged subset, not a symlink to the repo root.
...
The package path must be a real directory. Do not point it back to the repository root with a
symlink, because users may keep private gitignored data under `tenants/`, `outputs/`, `packets/`,
or `submissions/`.

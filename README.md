...
`./plugins/job-application-quality`, a materialized repo-local package path. Do not use a sparse
checkout that only includes `.agents/plugins`; the local plugin source must be present in the
marketplace clone. The package is intentionally not a symlink to the repo root, so gitignored local
data directories such as `tenants/`, `outputs/`, `packets/`, and `submissions/` are not part of the
Codex plugin source.
...
+-- plugins/
|   +-- job-application-quality/            # Materialized Codex plugin package
+-- .claude-plugin/
